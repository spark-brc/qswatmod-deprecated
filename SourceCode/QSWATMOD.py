...
/***************************************************************************
 ...
 This plugin displays the result of SM simulation
                              -------------------
        begin                : 2017-08-02  -- recent updated: 2019-02-13
        git sha              : $Format:%H$
        copyright            : (C) 2017 by Seonggyu and Anders
...
from PyQt4.QtCore import QSettings, QTranslator, qVersion, QCoreApplication, QFileInfo, QFileSystemWatcher
from PyQt4.QtCore import *
from PyQt4.QtGui import * # @UnusedWildImport
from PyQt4 import QtCore, QtGui, QtSql
from PyQt4.QtSql import QSqlDatabase, QSqlQuery, QSqlTableModel
from qgis.core import QgsVectorLayer, QgsField, QgsMapLayerRegistry, QgsFeatureIterator, QgsRasterLayer
...
class QSWATMOD:
    ...
    def __init__(self, iface):
        ...
        self.actions = []
        self.menu = self.tr(u'&QSWATMOD')
        # TODO: We are going to let the user set this up in a future iteration
        self.toolbar = self.iface.addToolBar(u'&QSWATMOD')
        self.toolbar.setObjectName(u'&QSWATMOD')
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('&QSWATMOD', message)
    ...
    def add_action(
        ...
        icon = QIcon(':/newPrefix/pics/logo.png')
    ...
    def initGui(self):
        ...
        icon_path = ':/newPrefix/pics/logo.png'
        self.add_action(
            icon_path,
            text=self.tr(u''),
            callback=self.run,
    ...
    def run(self):
        ...
        # ---------------------------------------------------------------------------------------------
        QObject.connect(self.dlg.tabWidget, SIGNAL('currentChanged(int)'), self.check_outputs)
        QObject.connect(self.dlg.tabWidget, SIGNAL('currentChanged(int)'), self.define_sim_period)
        # ---------------------------------------------------------------------------------------------
        # Run
    ...
    def import_mf_grid(self):
        ...
        self.dlg.textEdit_sm_link_log.append(time+' -> ' + 'MODFLOW grid shapefile has been imported ...')
    ...
    ### the following is used if we want the user to navigate to a MODFLOW model directory. is unused      
    ...
    #       Out_folder_temp = QgsProject.instance().readPath("./")
    #       Out_folder = os.path.normpath(Out_folder_temp + "/" + Project_Name + "/" + "QSWATMOD")
    #       #distutils.dir_util.remove_tree(Out_folder)
    #       distutils.dir_util.copy_tree(directory, Out_folder)
    #       #self.dlg.Lakemodel_directory.setText(Out_folder)
    #       #self.DB_Update_Lakemodel_Directory()
            
    #       file_check = os.path.abspath(os.path.join(Out_folder, "*.dis"))
    ...
    def hru_shapefile(self):            
        ...
        if inFileName:
            ...
            if os.name == 'nt':
                hru_shp = ntpath.join(Out_folder, inName + ".shp")
            else:
                hru_shp = posixpath.join(Out_folder, inName + ".shp")
            ...
            self.dlg.lineEdit_hru_shapefile.setText(hru_shp)
            self.mfOptionOn() # enable
    ...
    def subbasin_shapefile(self):
        ...
        if inFileName:
            ...
            for f in glob.iglob(pattern):
                ...
                shutil.copy(f, outfile)
            if os.name == 'nt':
                sub_shp = ntpath.join(Out_folder, inName + ".shp")
            else:
                sub_shp = posixpath.join(Out_folder, inName + ".shp")       
            layer = QgsVectorLayer(sub_shp, '{0} ({1})'.format("sub","SWAT"), 'ogr')
            ...
            QgsMapLayerRegistry.instance().addMapLayer(layer, False)
            swat_group.insertChildNode(0, QgsLayerTreeLayer(layer))
            self.dlg.lineEdit_subbasin_shapefile.setText(sub_shp)
            self.mfOptionOn() # enable
            post_i_str.read_sub_no(self)
    ...
    def river_shapefile(self):
        ...
        if inFileName:
            ...
            if os.name == 'nt':
                ...
                riv_shp = ntpath.join(Out_folder, inName + ".shp")
            else:
                riv_shp = posixpath.join(Out_folder, inName + ".shp")
            
            
            layer = QgsVectorLayer(riv_shp, '{0} ({1})'.format("riv","SWAT"), 'ogr')
            ...
            QgsMapLayerRegistry.instance().addMapLayer(layer, False)
            swat_group.insertChildNode(0, QgsLayerTreeLayer(layer))
            self.dlg.lineEdit_river_shapefile.setText(riv_shp)
            self.mfOptionOn() # enable
    ...
    def createMF(self):
        ...
        '''
        self.use_sub_shapefile()
        time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')       
        self.dlg.textEdit_sm_link_log.append(time+' -> ' + 'Sub is dissolved ...')      
        self.dlg.progressBar_sm_link.setValue(50)

        # Create MODFLOW active
        class_mf = createMFmodelDialog(self) # make object from createMF class
        class_mf.create_mf_act_grid()
        time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')       
        self.dlg.textEdit_sm_link_log.append(time+' -> ' + 'MODFLOW active grid is created ...')        
        self.dlg.progressBar_sm_link.setValue(80)

        # Extract fitted (mean) DEM from DEM (SWAT) to active grid
        class_mf.getElevfromDem()
        time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')       
        self.dlg.textEdit_sm_link_log.append(time+' -> ' + 'Extracting DEM and   ...')      
        self.dlg.progressBar_sm_link.setValue(90)

        # Delete mf_act_grid_null
        class_mf.mf_act_grid_delete_NULL()
        time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')               
        self.dlg.progressBar_sm_link.setValue(95)       # Convert mf_act_dem to raster

        # class_mf.cvtElevToR() -> not working due to the use of spinbox objects
        self.cvtElevToR()
        time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')       
        self.dlg.textEdit_sm_link_log.append(time+' -> ' + 'MODFLOW active top elevation is converted to Raster ...')       
        self.dlg.progressBar_sm_link.setValue(100)
    ...
    def geoprocessing_prepared(self):
        ...
        self.dlg.progressBar_sm_link.setValue(20)
        time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')       
        self.dlg.textEdit_sm_link_log.append(time+' -> ' + 'HRU area is calculated ...')            
        QCoreApplication.processEvents()
        ...
        self.dlg.progressBar_sm_link.setValue(40)
        time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')       
        self.dlg.textEdit_sm_link_log.append(time+' -> ' + 'HRU is disaggregated ...')    
        QCoreApplication.processEvents() # it works as F5

        linking_process.create_dhru_id(self)    
        self.dlg.progressBar_sm_link.setValue(45)
        time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')       
        self.dlg.textEdit_sm_link_log.append(time+' -> ' + 'DHRU id is created ...')
        ...
        time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')       
        self.dlg.textEdit_sm_link_log.append(time+' -> ' + 'DHRU area is calculated ...')   
        QCoreApplication.processEvents() # it works as F5
       
        linking_process.hru_dhru(self)
        # linking_process.dissolve_hru_dhru(self)  # it causes a Null
        linking_process.create_hru_dhru_filter(self)    
        linking_process.delete_hru_dhru_with_zero(self)
        linking_process.hru_dhru_dissolve(self)
        self.dlg.progressBar_sm_link.setValue(50)
        time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')
        self.dlg.textEdit_sm_link_log.append(time+' -> ' + 'HRU is intersected by DHRU ...')    
        QCoreApplication.processEvents()
        ...
        self.dlg.progressBar_sm_link.setValue(80)
        QCoreApplication.processEvents()
        self.dlg.progressBar_sm_link.setValue(85)
        ...
        time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')
        self.dlg.textEdit_sm_link_log.append(time+' -> ' + 'DHRUs are intersected by GRIDs ...')    
        QCoreApplication.processEvents()
        ...
        self.dlg.progressBar_sm_link.setValue(100)
        time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')
        self.dlg.textEdit_sm_link_log.append(time+' -> ' + 'Tables have been exported to your Table folder!')    
        QCoreApplication.processEvents()
        ...
        questionBox.setWindowIcon(QtGui.QIcon(':/newPrefix/pics/logo.png'))
        ...
        if reply == QtGui.QMessageBox.Yes:
            ...
            self.dlg.tabWidget.setTabEnabled(2, True)
        ''' SWAT+
        linking_process.river_sub(self) #SWAT+
        linking_process.river_sub_delete_NULL(self) #SWAT+
        linking_process._create_river_sub_filter(self) #SWAT+
        linking_process._delete_river_sub_with_threshold(self) #SWAT+
        linking_process._rgrid_len(self) #SWAT+

        linking_process._export_hru_dhru(self)  
        linking_process._export_dhru_grid(self)
        linking_process._export_grid_dhru(self)
        linking_process._export_rgrid_len(self)
        '''        
        # Enable 3rd tap
        # for i in range(2, 5):
        #   self.dlg.tabWidget.setTabEnabled(i, False)
        ...
        self.define_sim_period()            
        # msgBox = QMessageBox()
        # msgBox.setText("disaggregating is done...")
        # msgBox.exec_()
    ...
    def test_table(self):
        ...
        #self.dlg.tableView_test.horizontalHeader().setResizeMode(QHeaderView.Stretch)
        #some of the columns are removed
        #Defining which columns that are hidden
        #for COL in range(0,12):
        #   self.dlg.tableView_test.setColumnHidden(COL, True)
        #self.dlg.tableView_test.setColumnHidden(24, True)
