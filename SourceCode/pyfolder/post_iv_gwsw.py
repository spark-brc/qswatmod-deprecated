...
def dissolvedSub(self):
    QSWATMOD_path_dict = self.dirs_and_paths()
    if not any(lyr.name() == ("mf_boundary (MODFLOW)") for lyr in QgsMapLayerRegistry.instance().mapLayers().values()):
...
def create_sm_riv(self):
    QSWATMOD_path_dict = self.dirs_and_paths()
    if not any(lyr.name() == ("sm_riv") for lyr in QgsMapLayerRegistry.instance().mapLayers().values()):
        ...
        input2 = QgsMapLayerRegistry.instance().mapLayersByName("river_grid (SWAT-MODFLOW)")[0]
        ...
        input1.setSelectedFeatures(riv_mat)
        name_ext = "sm_riv.shp"
...
def plot_gwsw(self):
    ...
    river = shapefile.Reader(os.path.join(orgGIS, "riv_SM.shp" )) # River
    sub = shapefile.Reader(os.path.join(orgGIS, "mf_boundary.shp" )) # dissolved sub
    sm_riv = shapefile.Reader(os.path.join(smGIS, "sm_riv.shp"))
...
def export_gwsw(self):
    ...
    if self.dlg.radioButton_gwsw_day.isChecked():
        with open(os.path.join(outfolder, "GWSW(" + str(selectedDate) + ")_daily.txt"), 'w') as f:
            f.write("# GWSW(" + str(selectedDate) + ")_daily - QSWATMOD Plugin " + version + time + "\n")
            df.to_csv(
                f,
                # index_label="Date",
                index=False,
        ...
        with open(os.path.join(outfolder, "GWSW(" + str(selectedDate) + ")_monthly.txt"), 'w') as f:
            f.write("# GWSW(" + str(selectedDate) + ")_monthly - QSWATMOD Plugin " + version + time + "\n")
            df.to_csv(
                f,
                # index_label="Date",
                index=False,
        ...
        with open(os.path.join(outfolder, "GWSW(" + str(selectedDate) + ")_annual.txt"), 'w') as f:
            f.write("# GWSW(" + str(selectedDate) + ")_annual - QSWATMOD Plugin " + version + time + "\n")
            df.to_csv(
                f,
                # index_label="Date",
                index=False,
...
def create_gwsw_shp(self):
    ...
    sm_riv = QgsMapLayerRegistry.instance().mapLayersByName("sm_riv")[0]
    ...
    if self.dlg.radioButton_gwsw_day.isChecked():
        ...
        if not any(lyr.name() == (name) for lyr in QgsMapLayerRegistry.instance().mapLayers().values()):
...
def export_gwswToShp(self):
    ...
    if self.dlg.radioButton_gwsw_day.isChecked():
        filename = "swatmf_out_MF_gwsw"
        self.layer = QgsMapLayerRegistry.instance().mapLayersByName("gwsw_day")[0]
        ...
        filename = "swatmf_out_MF_gwsw_monthly"
        self.layer = QgsMapLayerRegistry.instance().mapLayersByName("gwsw_monthly")[0]
        nSM_riv = self.layer.featureCount()
        data = np.loadtxt(
                        os.path.join(wd, filename),
                        skiprows=2,
        ...
        filename = "swatmf_out_MF_gwsw_yearly"
        self.layer = QgsMapLayerRegistry.instance().mapLayersByName("gwsw_yearly")[0]
        nSM_riv = self.layer.featureCount()
        data = np.loadtxt(os.path.join(wd, filename),
                          skiprows=2,
                          comments=["year:", "Layer"])
    ...
    for i in dateList_f:
        ...
        date_idx = provider.fields().fieldNameIndex(i)
