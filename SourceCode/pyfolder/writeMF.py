...
def createBotElev(self):
    self.layer = QgsMapLayerRegistry.instance().mapLayersByName("mf_act_grid (MODFLOW)")[0]
...
def cvtBotElevToR(self):
    ...
    input2 = QgsMapLayerRegistry.instance().mapLayersByName("top_elev (MODFLOW)")[0]
    ...
    self.time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')
    self.textEdit_mf_log.append(self.time+' -> ' + 'Aquifer thickness is converted to Raster ...')
...
def getBotfromR(self):
    ...
    input1 = QgsMapLayerRegistry.instance().mapLayersByName("mf_act_grid (MODFLOW)")[0]
    input2 = QgsMapLayerRegistry.instance().mapLayersByName("bot_elev (DATA)")[0]
    ...
    zoneStat = QgsZonalStatistics(input1, rpath, 'bot_', 1, QgsZonalStatistics.Mean)
...
def cvtHKtoR(self):
    ...
    mf_group.insertChildNode(0, QgsLayerTreeLayer(layer))
    time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')
    self.textEdit_mf_log.append(time+' -> ' + 'Horizantal Hydraulic Conductivity has been converted to Raster ...')
...
def getHKfromR(self):
    from qgis.analysis import QgsZonalStatistics
    input1 = QgsMapLayerRegistry.instance().mapLayersByName("mf_act_grid (MODFLOW)")[0]
    input2 = QgsMapLayerRegistry.instance().mapLayersByName("hk (DATA)")[0]
    provider1 = input1.dataProvider()
    provider2 = input2.dataProvider()
    rpath = provider2.dataSourceUri()
    ...
    if provider1.fields().indexFromName("hk_mean") != -1:
        ...
        provider1.deleteAttributes([attrIdx])
    zoneStat = QgsZonalStatistics(input1, rpath, 'hk_', 1, QgsZonalStatistics.Mean)
    zoneStat.calculateStatistics(None)
    time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')
...
def getSSfromR(self):
    from qgis.analysis import QgsZonalStatistics
    input1 = QgsMapLayerRegistry.instance().mapLayersByName("mf_act_grid (MODFLOW)")[0]
    input2 = QgsMapLayerRegistry.instance().mapLayersByName("ss (DATA)")[0]
    provider1 = input1.dataProvider()
    provider2 = input2.dataProvider()
    rpath = provider2.dataSourceUri()
    ...
    if provider1.fields().indexFromName("ss_mean") != -1:
        ...
        provider1.deleteAttributes([attrIdx])
    zoneStat = QgsZonalStatistics(input1, rpath, 'ss_', 1, QgsZonalStatistics.Mean)
    zoneStat.calculateStatistics(None)
    time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')
...
def getSYfromR(self):
    from qgis.analysis import QgsZonalStatistics
    input1 = QgsMapLayerRegistry.instance().mapLayersByName("mf_act_grid (MODFLOW)")[0]
    input2 = QgsMapLayerRegistry.instance().mapLayersByName("sy (DATA)")[0]
    provider1 = input1.dataProvider()
    provider2 = input2.dataProvider()
    rpath = provider2.dataSourceUri()
    ...
    if provider1.fields().indexFromName("sy_mean") != -1:
        ...
        provider1.deleteAttributes([attrIdx])
    zoneStat = QgsZonalStatistics(input1, rpath, 'sy_', 1, QgsZonalStatistics.Mean)
    zoneStat.calculateStatistics(None)
    time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')
...
def getIHfromR(self):
    from qgis.analysis import QgsZonalStatistics
    input1 = QgsMapLayerRegistry.instance().mapLayersByName("mf_act_grid (MODFLOW)")[0]
    input2 = QgsMapLayerRegistry.instance().mapLayersByName("initialH (DATA)")[0]
    provider1 = input1.dataProvider()
    provider2 = input2.dataProvider()
    rpath = provider2.dataSourceUri()
    ...
    if provider1.fields().indexFromName("ih_mean") != -1:
        ...
        provider1.deleteAttributes([attrIdx])
    zoneStat = QgsZonalStatistics(input1, rpath, 'ih_', 1, QgsZonalStatistics.Mean)
    zoneStat.calculateStatistics(None)
    time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')
...
def getEVTfromR(self):
    from qgis.analysis import QgsZonalStatistics
    input1 = QgsMapLayerRegistry.instance().mapLayersByName("mf_act_grid (MODFLOW)")[0]
    input2 = QgsMapLayerRegistry.instance().mapLayersByName("evt (DATA)")[0]
    provider1 = input1.dataProvider()
    provider2 = input2.dataProvider()
    rpath = provider2.dataSourceUri()
    ...
    if provider1.fields().indexFromName("evt_mean") != -1:
        ...
        provider1.deleteAttributes([attrIdx])
    zoneStat = QgsZonalStatistics(input1, rpath, 'evt_', 1, QgsZonalStatistics.Mean)
    zoneStat.calculateStatistics(None)
    time = datetime.now().strftime('[%m/%d/%y %H:%M:%S]')
    self.textEdit_mf_log.append(time+' -> ' + 'Extrating Specific Yield from Raster has been finished...')
...
def writeMFmodel(self):
    ...
    if not self.lineEdit_createMFfolder.text():
        ...
        # top_elev
        top_elev = QgsMapLayerRegistry.instance().mapLayersByName("top_elev (MODFLOW)")[0]
        ...
        # bot_elev
        bot_elev = QgsMapLayerRegistry.instance().mapLayersByName("bot_elev (MODFLOW)")[0]
        ...
        if (self.radioButton_hk_single.isChecked() and self.lineEdit_hk_single.text()):
            ...
            hk = QgsMapLayerRegistry.instance().mapLayersByName("hk (MODFLOW)")[0]
        ...
        if (self.radioButton_ss_single.isChecked() and self.lineEdit_ss_single.text()):
            ...
            ss = QgsMapLayerRegistry.instance().mapLayersByName("ss (MODFLOW)")[0]
        ...
        if (self.radioButton_sy_single.isChecked() and self.lineEdit_sy_single.text()):
            ...
            sy = QgsMapLayerRegistry.instance().mapLayersByName("sy (MODFLOW)")[0]
        ...
        if self.groupBox_evt.isChecked():
            if (self.radioButton_evt_single.isChecked() and self.lineEdit_evt_single.text()):
                ...
                evt = QgsMapLayerRegistry.instance().mapLayersByName("evt (MODFLOW)")[0]
        ...
        # initialH
        initialH = QgsMapLayerRegistry.instance().mapLayersByName("initialH (MODFLOW)")[0]
        ...
        # mf_dem = QgsMapLayerRegistry.instance().mapLayersByName("top_elev (MODFLOW)")[0]
        ...
        # NWT solver -------------------------------------------------------------------
        nwt = flopy.modflow.ModflowNwt(mf, headtol=0.1, fluxtol=500, maxiterout=1000, Continue=True)
        ...
        ### Riv package =========================================================
        riv = QgsMapLayerRegistry.instance().mapLayersByName("mf_riv2 (MODFLOW)")[0]
        provider = riv.dataProvider()

        # Get the index numbers of the fields
        grid_id_idx = provider.fields().fieldNameIndex("grid_id")
        layer_idx = provider.fields().fieldNameIndex("layer")
